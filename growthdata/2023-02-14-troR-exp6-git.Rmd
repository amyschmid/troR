...
# metafile name
meta <- '220930pyrEtroRarsRTroRHAArsRHAfeantiox_metadata-AKS.csv'
# folder with data
folderdata <- '~/growthdata/data/'
# datafile name
data <- '220930pyrEtroRarsRTroRHArsHAfeantiox.csv'
...
#name the experiment
expt <- 'exp6_220930pyrEtroRarsRTroRHArsHAfeantiox'
...
stats2.fin <- stats2[!(stats2$condition == "-Fe+antiox" | stats2$condition =="100Fe+antiox" | stats2$name == "ArsRHA"), ]

#write output file for data compilation across experiments
write.csv (stats2.fin, file = "exp6_fitcurves.csv")

#make output file for data compilation
stats2.fin.pyr <- filter (stats2.fin, name == "pyrE")
write.csv (stats2.fin.pyr, file = "exp6_fitcurves_parent.csv")
```
...
pdf(paste(expt, "_fitcurves_exp6_CI95_no-antiox_plot.pdf", sep = ""), width = 8, height = 4)
plot(fitted.data.CI95)
dev.off()
...
## final plot of fitted curves with updated colors, sd
```{r}
stats2.fin$facet = factor(stats2.fin$condition, levels = c("-Fe", "100microMFe"))
#set colors
#mycols<-  c( pyrE="#7CAE00",  troR ="#C77CFF", arsR = "orange", troRHA = "#4D2570")
fin.cols<-c(pyrE="orangered4", arsR = "grey60", TroRHA = "medium blue", troR ="deepskyblue")
...
pdf(paste(expt, "_fitcurves_exp6_sd_no-antiox_plot-final-colors.pdf", sep = ""), width = 8, height = 4)
plot(fitted.data.sd)
dev.off()
...
write.csv(tmp, file = "exp6.growth.params.csv")
tmp.fin <-tmp[!(tmp$condition =="100Fe+antiox" | tmp$condition == "-Fe+antiox" | tmp$name == "ArsRHA"), ]
write.csv(tmp.fin, file = "exp6.growth.params.fin.csv")
```
...
## AUC plot without arsRHA, antioxidant
```{r}
params.dtx.fin <- params.dtx[!(params.dtx$name == 'ArsRHA' | params.dtx$condition == "-Fe+antiox" | params.dtx$condition == "100Fe+antiox"), ]
...
pdf(paste(expt, "_troRHA-AUC_plot-facets-fin-exp6.pdf", sep = ""), width = 8, height = 4)
plot(figure)
dev.off()
...
pdf(paste(expt, "troRHA_tmu_plot-facets-fin-exp6.pdf", sep = ""), width = 8, height = 4)
plot(figure)
dev.off()
```
...
write.table (pwt.mu, file = "exp6_ttest_mu.txt", sep = "\t")
write.table (pwt.auc, file = "exp6_ttest_auc.txt", sep = "\t")
```

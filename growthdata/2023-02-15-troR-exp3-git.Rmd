...
foldermeta <- '~/meta/'
# metafile name
meta <- '101915hfxhvo_meta-AKS.csv'
# folder with data
folderdata <- '~/data/'
# datafile name
data <- '101915hfxhvo-AKS.csv'
...
p_ave_con <-
ggplot(stats, aes(x = time, y = average, color = strain)) +
...
ggsave(p_ave_con,
      file = paste(
        format(Sys.time(), '%Y%m%d'),
        '_',
        'plot_average_condition_exp3',
        '.pdf',
        sep = ''),
      width = 4, height = 6, units = "in", device='pdf')
```

#Plot all average curves +/- sd, now grouped by condition
```{r}
# growth curves - average condition
p_ave_con <-
ggplot(stats, aes(x = time, y = average, color = strain)) +
...
ggsave(p_ave_con,
      file = paste(
        format(Sys.time(), '%Y%m%d'),
        '_',
        'plot_average_condition_sd-exp3',
        '.pdf',
        sep = ''),
      width = 4, height = 6, units = "in", device='pdf')
```
...
```{r}
# Truncate or trim the input data to observations occuring in the first 65 hours to fit accurately the exponential growth phase.
...
#remove Hme data, since they are not going to be included in the paper
stats2.fin <- stats2[!(stats2$strain == "HM33"), ]

#write output file for data compilation across experiments
write.csv (stats2.fin, file = "exp3_fitcurves-parent.csv")
```
## plot of fitted curves
```{r}
stats2.fin$facet = factor(stats2.fin$condition, levels = c("-Fe", "1microMFe", "100microMFe"))
#set colors
...
stats2.fin %>%
  ...
  #facet_wrap(~facet) +
  facet_grid(.~facet) +
  #scale_color_manual('condition', values = rainbow(5)) +
...
#pdf(paste(expt, "_fitcurves_exp3_CI95_plot.pdf", sep = ""), width = 8, height = 4)
plot(fitted.data.CI95)
#dev.off()
...
#make output file for combining stats together with all the bioscreen runs
tmp <- tmp[!(tmp$strain == "HM33"),]
write.csv (tmp, file = "exp3.params.noHM.csv", sep = ",")
```

## Now plot auc for each condition
```{r}
#  parameters compared to WT (by condition, not yet normalized to WT)

params.dtx$facet = factor(params.dtx$condition, levels = c( "-Fe", "1microMFe", "100microMFe"))
...
params.dtx %>%
  ...
  scale_fill_viridis("condition", discrete = TRUE, begin = 0.2, end = 0.8, direction = -1) +
...
## AUC plot without HME
```{r}
params.dtx.fin <- params.dtx[!(params.dtx$strain == 'HM33'), ]
#params.dtx.fin$facet = factor(params.dtx.fin$condition, levels = c("-Fe", "10microMFe" ,"100microMFe"))

#mycols<-  c( pyrE="#7CAE00",  troR ="#C77CFF", arsR = "orange", troRHA = "#4D2570")
  
params.dtx.fin %>%
  ggplot(aes(x = condition, y = avg_auc, fill = name)) +
  xlab("") +
  ylab("AUC") +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
  geom_errorbar(aes(ymin = avg_auc - auc_sd, ymax = avg_auc + auc_sd),
...
pdf(paste(expt, "_AUC_plot-fin-exp3.pdf", sep = ""), width = 8, height = 4)
plot(figure)
dev.off()
...
```{r}
params.dtx.fin %>%
  ggplot(aes(x = condition, y = avg_tgen, fill = name)) +
  xlab("") +
...
pdf(paste(expt, "td_plot-facets-fin.pdf", sep = ""), width = 8, height = 4)
plot(figure)
dev.off()
```
...
```{r}
params.dtx.fin %>%
  ggplot(aes(x = condition, y = avg_mu, fill = name)) +
  xlab("") +
  ylab(expression(mu)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +
...
pdf(paste(expt, "tmu_plot-fin-exp3.pdf", sep = ""), width = 8, height = 4)
plot(figure)
dev.off()
```

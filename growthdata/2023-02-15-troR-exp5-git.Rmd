...
foldermeta <- '~/meta/'
# metafile name
meta <- 'dtxRsiderophoreYPCPR_metadata-AKS.csv'
# folder with data
folderdata <- '~/data/'
# datafile name
data <- 'dtxRsiderophoreYPCPR.csv'
...
# growth curves - average strain, sd as error
suppressWarnings({
p_ave_str <-
...
#name the experiment
expt <- 'exp5_dtxRsiderophoreYPCPR'
...
#remove antioxidant conditions and arsRHA, since they are uninformative and too variable to interpret.
stats2.fin <- stats2[!(stats2$condition == ""), ]
```
## final plot of fitted curves
```{r}
stats2.fin$facet = factor(stats2.fin$condition, levels = c("-Fe", "100microMFe"))
#set colors
#mycols<-  c( pyrE="#7CAE00",  troR ="#C77CFF", idr = "", delta4445 = "",)
...
pdf(paste(expt, "_fitcurves_exp5_sd_plot.pdf", sep = ""), width = 8, height = 4)
plot(fitted.data.sd)
dev.off()
...
```{r}
#make output file for final data compilation of fitted growth curves

fits.fin <- stats2.fin[!(stats2.fin$strain == "delta41" | stats2.fin$strain == "delta4445" | stats2.fin$strain == "delta44" | stats2.fin$strain == "delta4144" | stats2.fin$condition == "" |  stats2.fin$strain == "arsRHA"), ]

write.csv (fits.fin, file = "exp5_fitcurves.csv")

#pyrE only for titration experiment results
fits.fin.pyr <- filter (fits.fin, name == "pyrE")
write.csv (fits.fin.pyr, file = "exp5_fitcurves_parent.csv")
...
tmp %>%
 ...
 dplyr:: summarise(
  ...
  group_by(condition, strain, name, media) %>% # average bioreps and calc sd
  dplyr::summarise(
    avg_tgen = mean(tgen),
    tgen_sd = sd(tgen),
    avg_mu = mean(mu),
...
write.csv(tmp, file = "exp5.growth.params.csv")
tmp.fin <-tmp[!(tmp$condition =="" | tmp$media == "YPC100" | tmp$media == "YPC-"), ]
tmp.fin.nosid <- tmp.fin[!(tmp.fin$strain == "delta41" | tmp.fin$strain == "delta4445" | tmp.fin$strain == "delta44" | tmp.fin$strain == "delta4144"), ]
write.csv(tmp.fin, file = "exp5.growth.params.fin.csv")
write.csv (tmp.fin.nosid, file = "exp5.growth.params.fin.nosid.csv")
```

## Now plot auc for WT and each dtx mutant
```{r}
#  parameters compared to WT (by condition, not yet normalized to WT)
params.dtx$facet = factor(params.dtx$condition, levels = c( "-Fe", "100microMFe"))
#subset to exclude siderophore mutants (which don't show a phenotype)
...
## AUC plot without YPC conditions or siderophore mutants
```{r}
#filter out strains and conditions not included in the paper
params.dtx.fin <- params.dtx[!(params.dtx$strain == "delta41" | params.dtx$strain == "delta4445" | params.dtx$strain == "delta44" | params.dtx$strain == "delta4144" | params.dtx$condition == "" | params.dtx$media == "YPC100" | params.dtx$media == "YPC-"), ]
...
pdf(paste(expt, "_AUC_plot-facets-fin-exp5.pdf", sep = ""), width = 8, height = 4)
plot(figure)
dev.off()
...
pdf(paste(expt, "tmu_plot-facets-fin-exp5.pdf", sep = ""), width = 8, height = 4)
plot(figure)
dev.off()
```
...
write.table (pwt.mu, file = "exp5_ttest_mu.txt", sep = "\t")
write.table (pwt.auc, file = "exp5_ttest_auc.txt", sep = "\t")
```
